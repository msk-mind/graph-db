...
class ConfigSet():
    ...
    def add(self, path, value):
        '''
        Dynamically add objects to the configuration.

        :param path: a path that does not exist in the configuration. The path must be of the form
        ...
        :raises: ValueError if a configuration with the specified path already exists
        '''
        if self.has_value(path):
            raise ValueError('path {0} already exists in the configuration'.format(path))

        parsed = self._parse_path(path)
        name = parsed['name']
        jsonpath = parsed['jsonpath']

        ConfigSet.__INSTANCE.__config[name][jsonpath] = value
