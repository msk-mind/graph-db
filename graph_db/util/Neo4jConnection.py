...
class Neo4jConnection:
    ...
    def create_id_lookup_table_where(self, sqlc, source, sink, r="ID_LINK|HAS_RECORD", WHERE_CLAUSE=""):
        ...
        if re.compile('|'.join(banned_words), re.IGNORECASE).search(
                WHERE_CLAUSE):  # re.IGNORECASE is used to ignore case
        ...
        if result is None:
            ...
            return None
        cSchema = StructType([StructField(source, StringType(), True), StructField(sink, StringType(), True),
                              StructField("pathspec", StringType(), True)])
        return sqlc.createDataFrame(
            ([(x.data()['source']['value'], x.data()['sink']['value'], pretty_path(x.data()['path'])) for x in result]),
            schema=cSchema)
    ...
    def create_id_lookup_table(self, sc, sqlc, SOURCE_TYPE, SINK_TYPE, QUERY_ID):
        ...
        return sqlc.createDataFrame(
            sc.parallelize([(x.data()['source']['value'], x.data()['sink']['value']) for x in result]), schema=cSchema)
